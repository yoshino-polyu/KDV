...
__device__ void dSearchRange(const CUDA_KDNode *nodes, const int *indexes,
                             const Point *pts, const Point &query,
                             const float range, int &ret_num_nbrs,
                             int *ret_indexes, float *ret_dists) {
  // printf("begin dSearchRange!\n");
  //  Goes through all the nodes that are within "range"
  int cur = 0;  // root
  int num_nbrs = 0;

  // Ok, we don't have nice STL vectors to use, and we can't dynamically
  // allocate memory with CUDA?? We'll use a fixed length stack, increase this
  // as required
  ...
  while (to_visit_pos) {
    ...
    // No memcpy available??
    for (int i = 0; i < next_search_pos; i++) to_visit[i] = next_search[i];
...
__global__ void CalcEdgeCorrectionWeights(float h2, const SamplePoints dPoints,
                                          const AsciiRaster dAscii,
                                          float *dWeights) {
  // serial point ID
  ...
  if (NARROW) {
    int r = YCOORD_TO_ROW(p_y + SQRT_CUT_OFF_FACTOR * h, nRows, yLLCorner,
                          cellSize);
    row_lower = MAX(0, r);
    row_upper = MIN(nRows - 1, YCOORD_TO_ROW(p_y - SQRT_CUT_OFF_FACTOR * h,
                                             nRows, yLLCorner, cellSize));
    col_lower = MAX(
        0, XCOORD_TO_COL(p_x - SQRT_CUT_OFF_FACTOR * h, xLLCorner, cellSize));
    col_upper = MIN(nCols - 1, XCOORD_TO_COL(p_x + SQRT_CUT_OFF_FACTOR * h,
                                             xLLCorner, cellSize));
  }
...
// Edge correction with adaptive bandwidth (variable bandwidth at each point in
// dHs)
__global__ void CalcEdgeCorrectionWeights(float *dHs,
                                          const SamplePoints dPoints,
                                          const AsciiRaster dAscii,
                                          float *dWeights) {
  // serial point ID
  int tid = (blockIdx.y * gridDim.y + blockIdx.x) * blockDim.x + threadIdx.x;
...
__global__ void KernelDesityEstimation(float h2, const SamplePoints dPoints,
                                       const AsciiRaster dAscii,
                                       float *dWeights) {
  // serial point ID
  int tid = (blockIdx.y * gridDim.y + blockIdx.x) * blockDim.x + threadIdx.x;
...
// Kernel density estimation with adaptive bandwidth (variable bandwidth at each
// point in dHs)
__global__ void KernelDesityEstimation(float *dHs, const SamplePoints dPoints,
                                       const AsciiRaster dAscii,
                                       float *dWeights) {
  // serial point ID
  int tid = (blockIdx.y * gridDim.y + blockIdx.x) * blockDim.x + threadIdx.x;
...
__global__ void DensityAtPoints(float h2, const SamplePoints dPoints,
                                float *dWeights, float *dDen0, float *dDen1) {
  // serial point ID
  int tid = (blockIdx.y * gridDim.y + blockIdx.x) * blockDim.x + threadIdx.x;
...
// Density at each point under fixed bandwidth h2 (squared)
///*
__global__ void DensityAtPointsKdtr(const CUDA_KDNode *nodes,
                                    const int *indexes, const Point *pts,
                                    float h2, const SamplePoints dPoints,
                                    float *dWeights, float *gpuDen) {
  ...
  dSearchRange(nodes, indexes, pts, query, range, n_NBRS, gpu_ret_indexes,
  gpu_ret_dist);
  ...
  */
  ...
  // call to dSearchRange requires too much memory for gpu_ret_indexes and
  // gpu_ret_dist. Embeding the code directly instead
  ////////////////////////////////////////////////////////////////////////////
  float g, tmp;
  float p_w = dPoints.weights[tid];
  float e_w = dWeights[tid];
...
__global__ void dCopyDensityValues(const SamplePoints dPoints, float *dWeights,
                                   const float h2, float *gpuDen, float *dDen0,
                                   float *dDen1) {
  // serial point ID
  int tid = (blockIdx.y * gridDim.y + blockIdx.x) * blockDim.x + threadIdx.x;
  // directly return if ID goes out of range
...
// brute force approach
// Density at each point under adaptive bandwidth (variable bandwidth at each
// point in dHs)
__global__ void DensityAtPoints(float *dHs, const SamplePoints dPoints,
                                float *dWeights, float *dDen0, float *dDen1) {
  // serial point ID
  int tid = (blockIdx.y * gridDim.y + blockIdx.x) * blockDim.x + threadIdx.x;
...
__global__ void DensityAtPointsKdtr(const CUDA_KDNode *nodes,
                                    const int *indexes, const Point *pts,
                                    float *dHs, const SamplePoints dPoints,
                                    float *dWeights, float *gpuDen) {
  // serial point ID
  int tid = (blockIdx.y * gridDim.y + blockIdx.x) * blockDim.x + threadIdx.x;
...
__global__ void dCopyDensityValues(const SamplePoints dPoints, float *dWeights,
                                   float *dHs, float *gpuDen, float *dDen0,
                                   float *dDen1) {
  // serial point ID
  int tid = (blockIdx.y * gridDim.y + blockIdx.x) * blockDim.x + threadIdx.x;
  // directly return if ID goes out of range
...
__global__ void CalcVaryingBandwidths(const SamplePoints dPoints, float h,
                                      float *dHs) {
  // serial point ID
  int tid = (blockIdx.y * gridDim.y + blockIdx.x) * blockDim.x + threadIdx.x;
...
// compute spatially varying bandwidths
__global__ void CalcVaryingBandwidths(const SamplePoints dPoints, float *dDen0,
                                      float h, float alpha, float *dHs) {
  // serial point ID
  int tid = (blockIdx.y * gridDim.y + blockIdx.x) * blockDim.x + threadIdx.x;
  ...
  //	den = dDen0_0;
  // }
  // float tmph = (h * (powf(expf(den) / g, alpha))); // this outmost () is
  // NECESSARY!
...
// **===------------------------------------------------------------------===**
__global__ void ReductionSum(float *g_data, unsigned int N, int iteration,
                             int num_active_items) {
  // use shared memory
  __shared__ float s_data[BLOCK_SIZE];

  unsigned int thread_id = threadIdx.x;
  unsigned int serial_thread_id =
      (blockIdx.y * gridDim.y + blockIdx.x) * blockDim.x + threadIdx.x;
  ...
  if (thread_id == 0) {
    unsigned int idx = (blockIdx.y * gridDim.y + blockIdx.x) * blockDim.x *
                       powf(blockDim.x, iteration);
    if (idx < N) {
      /*if(idx == 0){
              dDen0_0 = g_data[idx];
      }*/
...
// Mark the cells on boundary with 1 on a raster representation of the study
// area By Guiming @ 2016-09-02
__global__ void dMarkBoundary(AsciiRaster dAscii) {
  ...
  if (row == 0 || (row == nRows - 1) || col == 0 ||
      (col == nCols - 1)) {  // cells on the outmost rows and cols
...
__global__ void dCalcDist2Boundary(SamplePoints dPoints,
                                   const AsciiRaster dAscii) {
  // serial point ID
  int tid = (blockIdx.y * gridDim.y + blockIdx.x) * blockDim.x + threadIdx.x;
...
// for testing KD-tree for Hoption==0
__global__ void KernelDensityEstimationKdTree(const CUDA_KDNode *nodes,
                                              const int *indexes,
                                              const Point *pts, float h2,
                                              const SamplePoints dPoints,
                                              float *dWeights, float *gpuDen) {
  // 每个线程对应一个 sample point，索引 tid
  int tid = blockIdx.x * blockDim.x + threadIdx.x;
  int n = dPoints.numberOfPoints;
  if (tid >= n) return;

  // 当前 sample point 坐标、权重及边缘校正权重
  float p_x = dPoints.xCoordinates[tid];
  float p_y = dPoints.yCoordinates[tid];
  float p_w = dPoints.weights[tid];
  float e_w = dWeights[tid];

  // 构造查询点，就是当前 sample point
  Point query;
  query.coords[0] = p_x;
  query.coords[1] = p_y;

  // 定义搜索范围，通常为 CUT_OFF_FACTOR * h2（h2为带宽的平方）
  float range = CUT_OFF_FACTOR * h2;

  // 用固定长度栈模拟递归遍历 KD-Tree
  int to_visit[CUDA_STACK];
  int stackPos = 0;
  // 从根节点开始搜索（假设根节点下标为 0）
  to_visit[stackPos++] = 0;
  ...
  while (stackPos > 0) {
    int cur = to_visit[--stackPos];
    // 如果为叶子节点，则直接遍历该节点内保存的样本点索引
    if (nodes[cur].left == -1) {
      for (int i = 0; i < nodes[cur].num_indexes; i++) {
        int idx = indexes[nodes[cur].indexes + i];
        // 计算查询点与当前 kd-tree 中存储点的平方距离
        float dx = query.coords[0] - pts[idx].coords[0];
        float dy = query.coords[1] - pts[idx].coords[1];
        float d2 = dx * dx + dy * dy;
        if (d2 < range) {
          // 使用高斯核函数，kernel 值定义为： exp(-d2/(2*h2))/(h2*TWO_PI)
          float kernelVal = expf(-d2 / (2.0f * h2)) / (h2 * TWO_PI);
          // 贡献值乘以当前 sample point 的权重与边缘校正权值
          float contribution = kernelVal * p_w * e_w;
          // 对于该邻近点（在 kd-tree 中的，下标 idx），使用原子加累加贡献
          atomicAdd(&gpuDen[idx], contribution);
        }
      }
    } else {
      // 内部节点，根据当前节点切分轴决定进入哪个子树
      int split_axis = nodes[cur].level % KDTREE_DIM;
      float diff = query.coords[split_axis] - nodes[cur].split_value;
      // 如果 diff 的平方大于 range，则仅搜索一侧，否则两侧都搜索
      if (diff * diff > range) {
        if (diff < 0)
          to_visit[stackPos++] = nodes[cur].left;
        else
          to_visit[stackPos++] = nodes[cur].right;
      } else {
        to_visit[stackPos++] = nodes[cur].left;
        to_visit[stackPos++] = nodes[cur].right;
      }
